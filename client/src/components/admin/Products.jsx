...
const Products = ({ cat, filters, sort }) => {
  ...
  return (
    <>
      ...
      <Container>
        {cat
          ? filteredProducts.map((item) => <Product toggle={toggle} setToggle={setToggle} item={item} key={item.id} />)
          : products
            .slice(0, 8)
            .map((item) => <Product toggle={toggle} setToggle={setToggle}  item={item} key={item.id} />)}
      </Container>
    </>
  );
};
